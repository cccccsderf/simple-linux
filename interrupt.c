...
// 定义中断处理程序数组，在kernel.S中的处理程序
// 支持中断处理程序的入口，最终调用的是ide_table中的处理程序
...
static void pic_init(void){
    ...
    outb (PIC_S_DATA, 0x28); // ICW2:起始中断向量号为0x20
...
static void general_intr_handler(uint8_t vec_nr){
    ...
    // IRQ7和IRQ5会产生伪中断（spurious interrupt），无需处理
    {
        /* code */
        return;
    }
...
static void exception_init(void){
    ...
    for (i = 0; i < IDT_DESC_CNT; i++)
    {
        ...
        /* 默认为general_intr_handler以后会由register_handler来注册具体处理函数
         */
        intr_name[i] = "unknown"; //先同意赋值为unknown
    }
