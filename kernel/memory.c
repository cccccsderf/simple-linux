...
struct pool {
    ...
    uint32_t phy_addr_start;   //本内存池所管理内存的起始地址
...
struct mem_block_desc kk_block_descs[DESC_CNT]; // 内核内存块描述符数组

struct pool kernel_pool, user_pool; //内核内存池和用户内存池
struct virtual_addr kernel_vaddr;   //此结构用来给内核分配虚拟地址

    /* 在pf表示的虚拟地址内存池中申请pg_cnt个虚拟页
...
static void* vaddr_get(enum pool_flags pf, uint32_t pg_cnt){
    ...
    if (pf == PF_KERNEL){
        ...
        /* (0xc0000000 - PG_SIZE)作为用户三级栈已经在start_process被分配 */
...
uint32_t* pte_ptr(uint32_t vaddr){
    // 先访问页表自己 再访问目录项pde（页目录内页表的索引）作为pte的索引访问到页表
    // 再用pte的索引作为页内偏移
...
/* 再m_pool指向的物理内存中分配一个物理页，成功则返回页框的物理地址，失败则返回null */
...
static void page_table_add(void* _vaddr, void* _page_phyaddr){
    ...
    /****************************
     * 执行*pte，会访问空的pde，所以确保pde创建完成后才能执行pte
     * 否则会引发page_fault，因此pde位0时，pte只能出现在下面else语块中pde后面
    */
    ...
    *pde = (pde_phyaddr | PG_US_U | PG_RW_W | PG_P_1);
    /***************************
     * 分配到的物理页地址pde_pyhaddr对应的物理内存清0，
     * 避免里面的旧数据变成了页表项，从而让页表混乱
     * 访问pde对应的物理地址，用pte取高20位便可以
...
void* malloc_page(enum pool_flags pf, uint32_t pg_cnt){
    ...
    /*************************
     * 1 通过vaddr_get再虚拟内存池中申请虚拟地址
     * 2 通过palloc再物理内存池中申请物理页
     * 3 通过page_table_add将以上得到的虚拟地址和物理地址再页表中完成映射
    */
   ...
   while(cnt-- > 0){
    ...
    if(page_phyaddr == NULL){
        // 失败时要将曾经已经申请的虚拟地址和物理页全部回滚，在将来完成内存回收时再补充
        return NULL;
    }
    page_table_add((void*)vaddr, page_phyaddr); // 再页表中做映射
    vaddr += PG_SIZE;   // 下一个虚拟页
   }
   return vaddr_start;
}
    /*******************
     * 从内核物理内存中申请1页内存，成功返回其虚拟地址，失败返回null
    */
...
void* get_a_page(enum pool_flags pf, uint32_t vaddr){
    ...
    void* page_pyhaddr = palloc(mem_pool);
    if (page_pyhaddr == NULL)
    {
        return NULL;
    }
    page_table_add((void*)vaddr, page_pyhaddr);
...
static void mem_pool_init(uint32_t all_mem){
    ...
    // 页表大小=1页的页目录表+第0和第768个页目录指向同一个页表+
    // 第769-1022个页目录共指向254个页表，共256个页框
    
    uint32_t used_mem = page_table_size + 0x100000;
    uint32_t free_mem = all_mem - used_mem;
    uint16_t all_free_pages = free_mem / PG_SIZE;
    // 1页为4kb，不管总内存是不是4kb的倍数，对于以页为单位的内存分配策略，不足1页的内存不用考虑
    uint16_t kernel_free_pages = all_free_pages / 2;
    uint16_t user_free_pages = all_free_pages - kernel_free_pages;
    /* 为简化位图操作，余数不做处理，坏处是这样做会丢内存。
...
static void page_table_pte_remove(uint32_t vaddr){
    ...
    *pte &= ~PG_P_1; // 将页表项pte的p位置0
    asm volatile ("invlpg %0"::"m"(vaddr):"memory");//跟新tlb
}

/* 在虚拟地址池中释放_vader起始的连续pg_cnt个虚拟地址页 */
...
void block_desc_init(struct mem_block_desc* desc_array){
    ...
    for (desc_idx = 0; desc_idx < DESC_CNT; desc_idx++)
    {
        ...
        block_size *= 2;// 跟新下一个  32 64 128 512 1024
    }
...
void* sys_malloc(uint32_t size){
    ...
    if (cur_thread->pgdir == NULL)// 内核线程
    {
        ...
        mem_pool = &kernel_pool;
        descs = kk_block_descs;
    }else{// 用户进程pcb中的pgdir会为其分配页表时创建
        PF = PF_USER;
...
void mem_init(){
    ...
    mem_pool_init(mem_byte_total); // 初始化内存池
    block_desc_init(kk_block_descs);
