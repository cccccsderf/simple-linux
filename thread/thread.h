...
struct task_struct {
    ...
    /* 此任务自上次cpu运行后至今占用了多少cpu滴答数，任务执行了多久 */
    uint32_t elapsed_ticks;
    // 文件描述符数组
    int32_t fd_table[MAX_FILES_OPEN_PER_PROC];
    /* genneral_tag的作用是用户线程在一般的队列中的结点 */
    struct list_elem general_tag;
    /* all_list_tag是作用是用户线程队列thread_all_list中的结点 */
